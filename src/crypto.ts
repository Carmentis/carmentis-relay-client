...
export class AEADCrypto {
     ...
     * @returns Hex encoded key (URL-safe)
     */
     ...
     * @param keyHex - The hex encoded key
     * @returns Base64 encoded encrypted message (IV + ciphertext + auth tag)
     */
    static async encryptObject<T>(obj: T, keyHex: string): Promise<string> {
     ...
     */
    static async decryptObject<T>(encryptedBase64: string, keyHex: string): Promise<T> {
        try {
